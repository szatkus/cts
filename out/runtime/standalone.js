...
import { treeFromFilterResults } from '../framework/tree.js';
...
// DOM generation
function makeTreeNodeHTML(name, tree) {
  if (tree.children) {
  ...
  const runSubtree = async () => {
    ...
    if (res.logs) {
      ...
      for (const l of res.logs) {
        ...
        $('<button>').addClass('testcaselogbtn').attr('alt', 'Log stack to console').attr('title', 'Log stack to console').appendTo(caselog).on('click', () => {
          console.log(l);
        });
...
function makeSubtreeHTML(name, subtree) {
...
function makeSubtreeChildrenHTML(div, children) {
  const runSubtreeFns = [];
...
function makeTreeNodeHeaderHTML(name, description, runSubtree) {
  ...
  $('<button>').addClass('noderun').attr('alt', 'Run subtree').attr('title', 'Run subtree').on('click', async () => {
    await runSubtree();
    updateJSON();
  }).appendTo(div);
  $('<a>').addClass('nodelink').attr('href', href).attr('alt', 'Open').attr('title', 'Open').appendTo(div);
  const nodetitle = $('<div>').addClass('nodetitle').appendTo(div);
  $('<span>').addClass('nodename').html(nameHTML).appendTo(nodetitle);

  if (description) {
    $('<div>').addClass('nodedescription').html(description.replace(/\n/g, '<br>')).appendTo(nodetitle);
  }
